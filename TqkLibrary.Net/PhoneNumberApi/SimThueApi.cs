...
namespace TqkLibrary.Net.PhoneNumberApi
{
  ...
  public sealed class SimThueApi : BaseApi
  {
    ...
    public SimThueApi(string ApiKey, CancellationToken cancellationToken = default) : base(ApiKey,cancellationToken)
    {
    }

    public Task<SimThueBalanceResult> GetBalance()
      => RequestGet<SimThueBalanceResult>(string.Format(EndPoint + "/balance?key={0}", ApiKey));

    public Task<SimThueServicesResult> GetAvailableServices()
      => RequestGet<SimThueServicesResult>(string.Format(EndPoint + "/service?key={0}", ApiKey));
    ...
    public Task<SimThueRequestResult> CreateRequest(SimThueServiceResult serviceResult)
    {
      ...
      return RequestGet<SimThueRequestResult>(string.Format(EndPoint + "/create?key={0}&service_id={1}", ApiKey, serviceResult.Id));
    }
    ...
    public Task<SimThueCheckResult> CheckRequest(SimThueRequestResult createResult)
    {
      ...
      return RequestGet<SimThueCheckResult>(string.Format(EndPoint + "/check?key={0}&id={1}", ApiKey, createResult.Id));
    }

    public Task<SimThueRequestResult> CancelRequest(SimThueRequestResult createResult)
    {
      if (null == createResult) throw new ArgumentNullException(nameof(createResult));
      return RequestGet<SimThueRequestResult>(string.Format(EndPoint + "/cancel?key={0}&id={1}", ApiKey, createResult.Id));
    }
